...
Synopsis:
 ...
 <Plugin write_gcm>
   ...
   PrivateKeyFile "/etc/ssl/private/privatekey.p12"
   PrivateKeyPass "secretSslPassphrase"
 </Plugin>
...
The I<Project Number> of the GCE project. This should be a 12-digit decimal
number.
...
Defaults to
C<http://metadata/computeMetadata/v1/instance/service-accounts/default/token>.
...
Email address and PKCS#12 file to use for creating an authentication token.
This can be used when I<not> running on GCE, i.e. when the metadata server is
not available.

=item B<Url> I<Url>

URL of the GCM API. Defaults to
